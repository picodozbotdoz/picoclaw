...
ifeq ($(OS),Windows_NT)
        ...
        GIT_BRANCH_RAW:=$(strip $(shell git rev-parse --abbrev-ref HEAD 2>NUL))
        ...
        GIT_BRANCH_RAW:=$(strip $(shell git rev-parse --abbrev-ref HEAD 2>/dev/null))
...
GIT_BRANCH=$(if $(GIT_BRANCH_RAW),$(GIT_BRANCH_RAW),dev)
BUILD_TIME=$(if $(BUILD_TIME_RAW),$(BUILD_TIME_RAW),dev)
GO_VERSION=$(if $(GO_VERSION_RAW),$(GO_VERSION_RAW),unknown)
CONFIG_PKG=github.com/sipeed/picoclaw/pkg/config
LDFLAGS=-X $(CONFIG_PKG).Version=$(VERSION) -X $(CONFIG_PKG).GitCommit=$(GIT_COMMIT) -X $(CONFIG_PKG).GitBranch=$(GIT_BRANCH) -X $(CONFIG_PKG).BuildTime=$(BUILD_TIME) -X $(CONFIG_PKG).GoVersion=$(GO_VERSION) -s -w
