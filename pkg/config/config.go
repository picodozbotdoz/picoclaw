...
type TelegramSettings struct {
        ...
        EnableInline  bool            `json:"enable_inline"       yaml:"-"               env:"PICOCLAW_CHANNELS_TELEGRAM_ENABLE_INLINE"`
}
...
type ModelConfig struct {
        ...
        // ContextWindow overrides the agent-level context_window for this specific model.
        // When set to a non-zero value, it takes precedence over the heuristic
        // (MaxTokens * 4) and the AgentDefaults.ContextWindow setting.
        // This is essential for models with known context windows that differ
        // significantly from the default heuristic (e.g., DeepSeek V4 = 1M tokens).
        ContextWindow int `json:"context_window,omitempty" yaml:"context_window,omitempty"`

        // MaxOutputTokens is the maximum number of tokens the model can generate
        // in a single response. This is separate from MaxTokens (which controls the
        // per-request output limit). When set, it serves as a hard upper bound for
        // max_tokens to prevent accidental expensive requests (e.g., DeepSeek V4
