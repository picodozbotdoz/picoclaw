...
// RouterInterface is implemented by both Router and CostAwareRouter,
// allowing the agent loop to use either transparently.
...
// Verify Router implements RouterInterface at compile time.
...
// NewCostAware creates a CostAwareRouter when cost budget is configured.
// Falls back to a basic Router if no cost budget is set.
func NewCostAware(cfg RouterConfig, spend *SessionSpend) RouterInterface {
        if spend == nil {
                return New(cfg)
        }
        return NewCostAwareRouter(cfg, spend)
}
