...
// TestSingleSystemMessage verifies that BuildMessages produces at least one
// system message and that the first system message contains identity info.
// After the DeepSeek V4 context caching optimization, BuildMessages emits:
//   - messages[0] = system (stable): static prompt + stable contributors
//   - messages[1] = system (volatile): injected context + active skills + runtime
// so there are 1-2 system messages depending on whether volatile content exists.
// With dynamic context always present, there are always 2 system messages.
func TestSingleSystemMessage(t *testing.T) {
        ...
        for _, tt := range tests {
                t.Run(tt.name, func(t *testing.T) {
                        ...
                        // Must have at least 1 system message, first must be stable system
                        systemCount := 0
                        for _, m := range msgs {
                                if m.Role == "system" {
                                        systemCount++
                        ...
                        if systemCount < 1 {
                                t.Errorf("expected at least 1 system message, got %d", systemCount)
                        }
                        ...
                        // First system message must contain identity (static)
                        sys := msgs[0].Content
                        if !strings.Contains(sys, "picoclaw") {
                                t.Error("first system message missing identity")
                        }

                        // Dynamic context is in the second system message
                        var foundDynamicTime bool
                        for _, m := range msgs {
                                if m.Role == "system" && strings.Contains(m.Content, "Current Time") {
                                        foundDynamicTime = true
                                }
                        }
                        if !foundDynamicTime {
                                t.Error("no system message contains dynamic time context")
                        }

                        // Summary handling — emitted as user/assistant message pair
                        if tt.summary != "" {
                                var foundSummaryUser bool
                                for _, m := range msgs {
                                        if m.Role == "user" && strings.Contains(m.Content, "CONTEXT_SUMMARY:") {
                                                foundSummaryUser = true
                                        }
                                }
                                if !foundSummaryUser {
                                        t.Error("summary present but CONTEXT_SUMMARY user message missing")
                                }
                        } else {
                                for _, m := range msgs {
                                        if m.Role == "user" && strings.Contains(m.Content, "CONTEXT_SUMMARY:") {
...
func TestBuildMessages_CurrentSenderDynamicContext(t *testing.T) {
        ...
        for _, tt := range tests {
                t.Run(tt.name, func(t *testing.T) {
                        ...
                        // Concatenate all system messages to check for sender info
                        var allSystemContent string
                        for _, m := range msgs {
                                if m.Role == "system" {
                                        allSystemContent += m.Content
                                }
                        }
                        ...
                        if tt.wantSection {
                                ...
                                if !strings.Contains(allSystemContent, tt.wantLine) {
                                        t.Fatalf("system prompt missing sender line %q:\n%s", tt.wantLine, allSystemContent)
                                }
                                return
                        }
...
func TestBuildMessages_IncludesMediaOnlyCurrentMessage(t *testing.T) {
        ...
        // Find the user message with media (it's the last user message)
        var userMsg *providers.Message
        for i := range msgs {
                if msgs[i].Role == "user" && len(msgs[i].Media) > 0 {
                        userMsg = &msgs[i]
                        break
                }
        }
        if userMsg == nil {
                t.Fatal("no user message with media found")
        }
