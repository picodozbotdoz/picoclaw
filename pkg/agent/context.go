...
type ContextBuilder struct {
        ...
        // lastPrefixHash tracks the FNV-1a hash of the stable prompt prefix from
        // the previous BuildMessagesFromPrompt call. When the hash changes between
        // calls, it indicates a "cache break" — the prefix that the provider's
        // automatic prefix caching relies on has changed, and cached KV entries
        // are no longer valid. This is logged for observability.
        lastPrefixHash uint64
}
...
func (cb *ContextBuilder) BuildMessagesFromPrompt(req PromptBuildRequest) []providers.Message {
        ...
        if cacheBoundary > 0 {
                ...
                // Log when the prefix hash changes (cache break event).
                if cb.lastPrefixHash != 0 && cb.lastPrefixHash != prefixHash {
                        logger.InfoCF("agent", "Prefix cache break detected — stable prefix content changed",
                                map[string]any{
                                        "prev_hash": fmt.Sprintf("%016x", cb.lastPrefixHash),
                                        "new_hash":  fmt.Sprintf("%016x", prefixHash),
...
// computePrefixHash returns a simple fingerprint of the stable prompt prefix
// for cache hit validation. Uses FNV-1a for speed (not cryptographic).
// Two calls with identical prefix content will produce the same hash; a
// different hash means the prefix has changed and provider-side cached KV
// entries are stale.
func computePrefixHash(content string) uint64 {
        h := fnv.New64a()
