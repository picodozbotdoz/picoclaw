...
toolLoop:
        for i, tc := range normalizedToolCalls {
                ...
                if ts.agent.ContextPartition != nil {
                        budgets := ComputePartitionBudgets(ts.agent.ContextPartition, ts.agent.ContextWindow)
                        if budgets != nil {
                                execCtx = tools.WithInjectedContextBudget(execCtx, budgets.RetrievedContextTokens)
                        }
                }
