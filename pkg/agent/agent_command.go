        ...
        if agent != nil {
                ...
                rt.GetContextStats = func() *commands.ContextStats {
                        ...
                        return &commands.ContextStats{
                                ...
                                InjectedContextTokens: usage.InjectedContextTokens,
                                SystemPromptTokens:    usage.SystemPromptTokens,
                                HistoryTokens:         usage.HistoryTokens,
                                ToolDefTokens:         usage.ToolDefTokens,
                        }
                }

                // WS 4.3: Wire the /cost command to the agent's SessionCostTracker.
                rt.GetCostBreakdown = func() string {
                        if agent.CostTracker == nil {
                                return ""
                        }
                        breakdown := agent.CostTracker.CostBreakdown(agent.Model)
                        if agent.CostTracker.TotalCostUSD() == 0 {
                                return ""
                        }
                        return breakdown
                }
