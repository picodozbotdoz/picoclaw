...
func (al *AgentLoop) runTurn(ctx context.Context, ts *turnState, pipeline *Pipeline) (turnResult, error) {
        ...
        // Exploration phase: proactively gather context before the main loop.
        // Non-fatal: if exploration fails or is disabled, the turn continues.
        if _, exploreErr := pipeline.Explore(turnCtx, turnCtx, ts, exec); exploreErr != nil {
                logger.WarnCF("agent", "Exploration phase failed (non-fatal)",
                        map[string]any{"error": exploreErr.Error()})
        }
        ...
        for ts.currentIteration() < ts.agent.MaxIterations || len(exec.pendingMessages) > 0 || func() bool {
                ...
                case ControlBreak:
                        ...
                        // Verification phase: verify edits before finalizing.
                        // If verification fails, the error is injected as steering
                        // and we continue the loop so the LLM can fix it.
                        if !pipeline.Verify(ctx, turnCtx, ts, exec) {
                                exec.verificationRetries++
                                maxRetries := 2
                                if ts.agent.VerificationConfig != nil && ts.agent.VerificationConfig.MaxRetries > 0 {
                                        maxRetries = ts.agent.VerificationConfig.MaxRetries
                                }
                                if exec.verificationRetries <= maxRetries {
                                        // Inject verification failures as pending messages and continue the loop.
                                        ts.mu.RLock()
                                        followUps := append([]bus.InboundMessage(nil), ts.followUps...)
                                        ts.followUps = ts.followUps[:0]
                                        ts.mu.RUnlock()
                                        ...
                                        exec.pendingMessages = pendingMessages
                                        pendingMessages = nil
                                        continue
                                }
                                // Max retries exhausted: finalize with the current state.
                                logger.WarnCF("agent", "Verification max retries exhausted",
                                        map[string]any{"retries": exec.verificationRetries})
                        }
                ...
                case ControlToolLoop:
                        ...
                        case ToolControlBreak:
                                ...
                                if !pipeline.Verify(ctx, turnCtx, ts, exec) {
                                        ...
                                        if exec.verificationRetries <= maxRetries {
                                                // Inject verification failures as pending messages and continue.
                                                ts.mu.RLock()
                                                followUps := append([]bus.InboundMessage(nil), ts.followUps...)
                                                ts.followUps = ts.followUps[:0]
                                                ts.mu.RUnlock()
