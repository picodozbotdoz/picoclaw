...
func applyThinkingConfig(params *anthropic.MessageNewParams, level string) {
        ...
        if params.Temperature.Valid() {
                logger.InfoCF("provider", "Temperature cleared because thinking is enabled", map[string]any{"level": level})
        }
        ...
        if budget >= params.MaxTokens {
                logger.WarnCF("provider", "Budget tokens clamped", map[string]any{"budget": budget, "max_tokens": params.MaxTokens, "clamped_to": params.MaxTokens - 1})
                budget = params.MaxTokens - 1
        } else if budget > params.MaxTokens*80/100 {
                logger.WarnCF("provider", "Thinking budget exceeds 80% of max_tokens", map[string]any{"budget": budget, "max_tokens": params.MaxTokens})
        }
        ...
        for _, block := range resp.Content {
                ...
                case "tool_use":
                        ...
                        if err := json.Unmarshal(tu.Input, &args); err != nil {
                                logger.WarnCF("provider", "Failed to decode tool call input", map[string]any{"tool": tu.Name, "error": err.Error()})
