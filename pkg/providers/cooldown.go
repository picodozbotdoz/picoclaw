...
func (ct *CooldownTracker) MarkFailure(provider string, reason FailoverReason) {
        ...
        if reason == FailoverBilling {
                ...
                logger.WarnCF("provider", "Billing cooldown activated",
                        map[string]any{
                                "provider":        provider,
                                "billing_count":   billingCount,
                                "disabled_until":  entry.DisabledUntil.Format(time.RFC3339),
                                "cooldown":        calculateBillingCooldown(billingCount).String(),
                        })
        } else {
                entry.CooldownEnd = now.Add(calculateStandardCooldown(entry.ErrorCount))
                logger.WarnCF("provider", "Cooldown activated",
                        map[string]any{
                                "provider":      provider,
                                "reason":        string(reason),
                                "error_count":   entry.ErrorCount,
                                "cooldown_end":  entry.CooldownEnd.Format(time.RFC3339),
                                "cooldown":      calculateStandardCooldown(entry.ErrorCount).String(),
                        })
        }
}
...
func (ct *CooldownTracker) MarkSuccess(provider string) {
        ...
        wasInCooldown := !entry.CooldownEnd.IsZero() || !entry.DisabledUntil.IsZero()
        entry.ErrorCount = 0
        entry.FailureCounts = make(map[FailoverReason]int)
        ...
        if wasInCooldown {
                logger.InfoCF("provider", "Cooldown cleared after success",
                        map[string]any{
                                "provider": provider,
                        })
        }
