...
func NewHTTPClient(proxy string) *http.Client {
        ...
        if proxy != "" {
                ...
                if err == nil {
                        ...
                        logger.WarnCF("provider", "Invalid proxy URL", map[string]any{"proxy": proxy, "error": err.Error()})
                }
        }
        return client
}
...
func DecodeToolCallArguments(raw json.RawMessage, name string) map[string]any {
        ...
        if err := json.Unmarshal(raw, &decoded); err != nil {
                logger.WarnCF("provider", "Failed to decode tool call arguments payload", map[string]any{"tool": name, "error": err.Error()})
        ...
        case string:
                ...
                if err := json.Unmarshal([]byte(v), &arguments); err != nil {
                        logger.WarnCF("provider", "Failed to decode tool call arguments", map[string]any{"tool": name, "error": err.Error()})
                        arguments["raw"] = v
                }
                return arguments
        case map[string]any:
                return v
        default:
                logger.WarnCF("provider", "Unsupported tool call arguments type", map[string]any{"tool": name, "type": fmt.Sprintf("%T", decoded)})
                arguments["raw"] = string(raw)
                return arguments
        }
}
