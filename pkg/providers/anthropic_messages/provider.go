...
func (p *Provider) Chat(
        ...
        logger.DebugCF("provider", "Anthropic Messages API request",
                map[string]any{
                        "model":         model,
        ...
        if err != nil {
                logger.WarnCF("provider", "Anthropic Messages API request failed",
                        map[string]any{
                                "model": model,
                                "error": err.Error(),
                        })
        ...
        case http.StatusUnauthorized:
                logger.ErrorCF("provider", "Anthropic Messages API auth failed",
                        map[string]any{"model": model, "status_code": 401})
                return nil, fmt.Errorf("authentication failed (401): check your API key")
        case http.StatusTooManyRequests:
                logger.WarnCF("provider", "Anthropic Messages API rate limited",
                        map[string]any{"model": model, "status_code": 429, "latency_ms": latency.Milliseconds()})
                return nil, fmt.Errorf("rate limited (429): %s", string(body))
        case http.StatusBadRequest:
                logger.WarnCF("provider", "Anthropic Messages API bad request",
                        map[string]any{"model": model, "status_code": 400})
                return nil, fmt.Errorf("bad request (400): %s", string(body))
        case http.StatusNotFound:
                logger.WarnCF("provider", "Anthropic Messages API endpoint not found",
                        map[string]any{"model": model, "status_code": 404})
                return nil, fmt.Errorf("endpoint not found (404): %s", string(body))
        case http.StatusInternalServerError:
                logger.WarnCF("provider", "Anthropic Messages API server error",
                        map[string]any{"model": model, "status_code": 500})
                return nil, fmt.Errorf("internal server error (500): %s", string(body))
        case http.StatusServiceUnavailable:
                logger.WarnCF("provider", "Anthropic Messages API unavailable",
                        map[string]any{"model": model, "status_code": 503})
                return nil, fmt.Errorf("service unavailable (503): %s", string(body))
        default:
                if resp.StatusCode != http.StatusOK {
                        logger.WarnCF("provider", "Anthropic Messages API unexpected status",
                                map[string]any{"model": model, "status_code": resp.StatusCode})
                        return nil, fmt.Errorf("API request failed with status %d: %s", resp.StatusCode, string(body))
                }
        }

        // Parse response
        llmResp, parseErr := parseResponseBody(body)
        if parseErr != nil {
                logger.WarnCF("provider", "Anthropic Messages API response parse error",
                        map[string]any{"model": model, "error": parseErr.Error()})
                return nil, parseErr
        }

        logger.DebugCF("provider", "Anthropic Messages API response",
                map[string]any{
                        "model":             model,
...
func parseResponseBody(body []byte) (*LLMResponse, error) {
        ...
        for _, block := range resp.Content {
                ...
                case "tool_use":
                        ...
                        if err != nil {
                                logger.WarnCF("provider", "Failed to marshal tool call arguments",
                                        map[string]any{"tool_name": block.Name, "tool_id": block.ID, "error": err.Error()})
                        }
