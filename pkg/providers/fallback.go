...
func (fc *FallbackChain) Execute(
        ...
        for i, candidate := range candidates {
                ...
                if !fc.cooldown.IsAvailable(cooldownKey) {
                        ...
                        logger.WarnCF("provider", "Fallback candidate skipped: in cooldown",
                                map[string]any{
                ...
                if fc.rl != nil {
                        if !fc.rl.TryAcquire(cooldownKey) {
                                if i < len(candidates)-1 {
                                        logger.WarnCF("provider", "Fallback candidate skipped: rate limit saturated",
                                                map[string]any{
                                                        "candidate": cooldownKey,
                                                        "provider":  candidate.Provider,
                                                        "model":     candidate.Model,
                ...
                if failErr == nil {
                        ...
                        logger.WarnCF("provider", "Fallback: unclassified error, aborting",
                                map[string]any{
                                        "provider": candidate.Provider,
                                        "model":    candidate.Model,
                                        "error":    err.Error(),
                ...
                if !failErr.IsRetriable() {
                        logger.WarnCF("provider", "Fallback: non-retriable error, aborting",
                                map[string]any{
                                        "provider": candidate.Provider,
                                        "model":    candidate.Model,
                ...
                logger.WarnCF("provider", "Fallback: retriable error, trying next candidate",
                        map[string]any{
                                "provider":    candidate.Provider,
                                "model":       candidate.Model,
                                "reason":      string(failErr.Reason),
                ...
                if i == len(candidates)-1 {
                        logger.WarnCF("provider", "Fallback: all candidates exhausted",
                                map[string]any{
        ...
        logger.WarnCF("provider", "Fallback: all candidates skipped (cooldown)",
                map[string]any{
                        "attempts": len(result.Attempts),
                })
        return nil, &FallbackExhaustedError{Attempts: result.Attempts}
