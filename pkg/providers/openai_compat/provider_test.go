...
func TestProviderChat_DeepSeekOmitsReasoningContentForNonToolTurnHistory(t *testing.T) {
        ...
        _, err := p.Chat(t.Context(), messages, nil, "deepseek-chat", nil)
        if err != nil {
                t.Fatalf("Chat() error = %v", err)
        }
...
func TestProviderChat_HistoryCanonicalizationMatrix(t *testing.T) {
        ...
        captureRequestMessages := func(t *testing.T, providerName string, modelName string) []map[string]any {
                t.Helper()
                ...
                _, err := p.Chat(t.Context(), baseMessages, nil, modelName, nil)
                if err != nil {
                        t.Fatalf("Chat() error = %v", err)
                }
        ...
        t.Run("deepseek", func(t *testing.T) {
                msgs := captureRequestMessages(t, "deepseek", "deepseek-chat")
        ...
        t.Run("non-deepseek", func(t *testing.T) {
                msgs := captureRequestMessages(t, "", "test-model")
        ...
        t.Run("deepseek-v4", func(t *testing.T) {
                msgs := captureRequestMessages(t, "deepseek", "deepseek-v4-flash")
                if len(msgs) != len(baseMessages) {
                        t.Fatalf("len(messages) = %d, want %d", len(msgs), len(baseMessages))
                }

                // V4 preserves reasoning_content even in plain non-tool turns
                if msgs[1]["reasoning_content"] != "plain thought" {
                        t.Fatalf("turn1 reasoning_content = %v, want preserved for V4", msgs[1]["reasoning_content"])
                }
                if msgs[3]["reasoning_content"] != "tool thought" {
                        t.Fatalf("turn2 reasoning_content = %v, want preserved", msgs[3]["reasoning_content"])
                }
                // turn3 has no reasoning_content in the original message
                if _, ok := msgs[6]["reasoning_content"]; ok {
                        t.Fatalf("turn3 reasoning_content should be absent (was not in original), got %v", msgs[6]["reasoning_content"])
                }
                if msgs[9]["reasoning_content"] != "tool mixed thought" {
                        t.Fatalf("turn4 reasoning_content = %v, want preserved", msgs[9]["reasoning_content"])
                }
...
func TestProviderChat_DeepSeekV4PreservesReasoningContentForNonToolTurnHistory(t *testing.T) {
        // DeepSeek V4 models require reasoning_content to be preserved in ALL turns,
        // including non-tool turns, per the V4 API documentation: "When concatenating
        // subsequent multi-round messages, include the reasoning_content field to
        // maintain the context of the model's reasoning."
        var requestBody map[string]any
        ...
        if assistantMsg["reasoning_content"] != "Let me think... 1+1=2" {
                t.Fatalf(
                        "reasoning_content should be preserved for DeepSeek V4 non-tool turns, got %v",
                        assistantMsg["reasoning_content"],
                )
        }
}

func TestProviderChat_DeepSeekV4DocsReplayRequirements(t *testing.T) {
        // V4 models preserve reasoning_content in all turns including non-tool turns,
        // unlike V3 models which strip reasoning_content from plain turns.
        var requestBody map[string]any
        ...
        if plainAssistant["reasoning_content"] != "I know this from general knowledge." {
                t.Fatalf(
                        "V4 plain turn should preserve reasoning_content, got %v",
                        plainAssistant["reasoning_content"],
                )
        }
