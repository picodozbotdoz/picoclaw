...
type MessageBus struct {
        ...
        chatMemberEvents  chan ChatMemberEvent
...
func NewMessageBus() *MessageBus {
        return &MessageBus{
                ...
                chatMemberEvents: make(chan ChatMemberEvent, defaultBusBufferSize),
...
// PublishChatMemberEvent publishes a chat member status change event to the bus.
// These events are administrative (joins, leaves, promotions) and do not belong
// to a conversation session. Consumers use them for welcome messages, access
// control, and group management features.
func (mb *MessageBus) PublishChatMemberEvent(event ChatMemberEvent) error {
        if mb.closed.Load() {
                return ErrBusClosed
        }
        mb.wg.Add(1)
        ...
        case <-mb.done:
                return ErrBusClosed
        case mb.chatMemberEvents <- event:
                return nil
        }
}

// ChatMemberEventsChan returns a read-only channel for chat member events.
func (mb *MessageBus) ChatMemberEventsChan() <-chan ChatMemberEvent {
        return mb.chatMemberEvents
}
